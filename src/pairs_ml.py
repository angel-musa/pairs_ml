...
from coint_utils import test_cointegration_fullsample, rolling_coint_pvalues, calculate_spread_and_z, half_life
...
def parse_args():
    p = argparse.ArgumentParser(description="ML pairs: IBKR + Δspread + Purged CV + Adaptive β + Kalman + Meta")
...
def main():
    ...
    if len(tickers)!=2: raise SystemExit("Pass exactly two tickers, e.g. --tickers KO,PEP")
    use_rth = True if (args.use_rth or not args.all_hours) else False
    adjusted = True if (args.adjusted or not args.raw) else False

    cfg = dict(
        ticker_y=tickers[0], ticker_x=tickers[1],
        start_date=args.start, end_date=args.end,
        entry_threshold=args.entry_z, exit_threshold=args.exit_z,
        cost_per_leg=args.cost_bps/1e4, z_window=args.z_window,
        use_xgb=args.use_xgb,
        ib_host=args.host, ib_port=args.port, ib_client_id=args.client_id,
        use_rth=use_rth, adjusted=adjusted,
        beta_mode=args.beta_mode, beta_window=args.beta_window,
        rls_lam=args.rls_lam, kalman_q=args.kalman_q, kalman_r_scale=args.kalman_r_scale,
        coint_window=args.coint_window, coint_threshold=args.coint_threshold,
        coint_gate=(not args.no_coint_gate),
        time_stop=args.time_stop, vol_target=args.vol_target, vol_window=args.vol_window,
        z_cap=args.z_cap,
        meta=args.meta, meta_pt=args.meta_pt, meta_sl=args.meta_sl, meta_h=args.meta_h,
    ...
    print("="*60)
    print(f"Pair: {cfg['ticker_y']}/{cfg['ticker_x']}   Dates: {cfg['start_date']} → {cfg['end_date']}")
    print(f"EntryZ={cfg['entry_threshold']} ExitZ={cfg['exit_threshold']} Zwin={cfg['z_window']}  Cost/leg={cfg['cost_per_leg']*1e4:.1f} bps")
    print(f"β mode: {cfg['beta_mode']} (win={cfg['beta_window']} λ={cfg['rls_lam']} q={cfg['kalman_q']} rS={cfg['kalman_r_scale']})")
    print(f"Coint gate: {'ON' if cfg['coint_gate'] else 'OFF'} (win={cfg['coint_window']} p≤{cfg['coint_threshold']})")
    print(f"Time stop: {cfg['time_stop']}  Vol targeting: {'ON' if cfg['vol_target'] else 'OFF'}")
    print(f"Meta: {'ON' if cfg['meta'] else 'OFF'} (pt={cfg['meta_pt']} sl={cfg['meta_sl']} H={cfg['meta_h']} thr={cfg['meta_thresh']})")
    print(f"IB: {cfg['ib_host']}:{cfg['ib_port']} RTH={'Yes' if cfg['use_rth'] else 'All'} Adjusted={'Yes' if cfg['adjusted'] else 'Raw'}")
    ...
    # 6) Features & regressor (ΔS target)
    ...
    # 7) Optional meta-classifier (probability that the trade works)
    meta_proba = None
    if cfg["meta"]:
        print("\nStep 5: Meta-labeling (triple barrier)...")
        # Build predictions-based candidate set first
        # Reconstruct pred_z just for dataset building (same as backtest will)
        ...
        X_meta, y_meta = build_meta_dataset(
            feats.dropna(), pred_z.dropna(), z, entry_z=cfg["entry_threshold"],
            vol_window=cfg["vol_window"], pt_mult=cfg["meta_pt"], sl_mult=cfg["meta_sl"], max_h=cfg["meta_h"]
        )
        ...
        if clf is None:
            ...
            # Create a probability series for all timestamps where we might trade
            # (predict proba on the full feature matrix, align by index)
    ...
    # 8) Backtest (uses meta_proba if provided)
    print("\nStep 6: Backtest...")
    bt = backtest(
        y, x, beta, pred_dS, z,
        entry_z=cfg["entry_threshold"], exit_z=cfg["exit_threshold"],
        cost_per_leg=cfg["cost_per_leg"], z_window=cfg["z_window"],
        coint_mask=coint_mask, time_stop=cfg["time_stop"],
        vol_target=cfg["vol_target"], vol_window=cfg["vol_window"], z_cap=cfg["z_cap"],
        meta_proba=meta_proba, meta_threshold=cfg["meta_thresh"]
    )

    # 9) Diagnostics
    aligned = pd.concat([bt["pred_z"], z.shift(-1).reindex(bt["pred_z"].index)], axis=1, keys=["pred_z(t)","z(t+1)"]).dropna()
    if len(aligned)>20:
        print(f"Corr(pred_z(t), z(t+1)) = {aligned.corr().iloc[0,1]:.3f}")

    # 10) Plot
    print("\nStep 7: Plot...")
    plot_results(bt, z, cfg["entry_threshold"], cfg["exit_threshold"])
